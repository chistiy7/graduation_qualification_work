...
4. **GUI (Qt 6)** — вкладки «Задача» / «Результаты»: ввод как `--cli`, таблицы метрик, карта сетки, save/load JSON, экспорт отчёта.
5. **IO** — JSON `ScenarioBundle` (`scenario_json`); загрузка частичных сценариев из `data/scenarios/`.

Скриншоты для диплома: вкладка «Задача», «Планировка» (`LayoutGridWidget`), фрагмент «Стоимость», `output/plan.csv`.
